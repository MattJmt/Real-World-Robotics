...
def main():
    ...
    file_path_dof = 'policy_recordings/2023-12-15_14-59-04_dof_poses.npy'

    data = np.load(file_path_dof)
    env_nr = 1
    data_env = np.flip(data[env_nr], axis=0)
        ...
        while not rospy.is_shutdown() and viewer.is_running() and time.time() - start < 300:
            ...
            for joint_idx in range(len(d.ctrl)):
                ...
                if joint_idx == motornr_to_joint_dict["thumb2"]:
                    multiplier = 1
                    offset = np.deg2rad(-10)
                
                elif joint_idx == motornr_to_joint_dict["thumb1"]:
                    offset = np.deg2rad(6)
