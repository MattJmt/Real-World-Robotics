#!/usr/bin/env python3
import sys
sys.path.append('../../Kinematics/Code/')
sys.path.append('/home/mario/catkin_ws/src/Kinematics/Code/')
...
## Only if we implement a mujoco sim controller
...
class GripperControlNode:
    def __init__(self, sim=False, sub_queue_size=1) -> None:
        print("Initializing gripper control node")
        if not sim:
            self.gripper_controller = GripperController(calibration=False)
        else:
            print("No mujoco controller implemented yet")
            #self.gripper_controller = GripperControllerMujocoSim()
        # print("Connecting to dynamixels in gripper controller node")
    ...
    def write_gripper_angles(self, msg):
        ...
        print("the received message", unpacked_msg)
        self.gripper_controller.write_desired_joint_angles(unpacked_msg)
        self.last_received_gc = time.monotonic()


if __name__ == "__main__":
    print("Starting gripper control node")
